...
interface SupplyModalProps {
	...
	onChangeFormData: (data: { name: string; quantity: string; validity: string }) => void;
	onSave: () => void;
	onClose: () => void;
}
			...
			<View className="flex-1 items-center justify-center bg-black/50">
				<View className="w-11/12 max-w-md rounded-2xl bg-white p-6 shadow-xl">
					...
					<View className="gap-4">
						...
						<View>
							<Text className="mb-2 font-medium text-gray-700">Validade</Text>
							<TextInput
								className="rounded-lg border border-gray-300 px-4 py-3"
								value={formData.validity}
								onChangeText={(text) => onChangeFormData({ ...formData, validity: text })}
