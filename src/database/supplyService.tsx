import type { CreateSupplyDTO, Supply, UpdateSupplyDTO } from "@/interfaces/supply";
import { getDatabase } from "./initDatabase";

export class SupplyService {
	static async getAll(): Promise<Supply[]> {
		const db = await getDatabase();
		return db.getAllAsync<Supply>("SELECT * FROM supplies ORDER BY createdAt DESC");
	}

	static async getById(id: number): Promise<Supply | null> {
		const db = await getDatabase();
		const result = await db.getFirstAsync<Supply>("SELECT * FROM supplies WHERE id = ?", [id]);
		return result || null;
	}
    ...
    `,
		...
		const newItem = await SupplyService.getById(insert.lastInsertRowId);
	...
	static async update(id: number, data: UpdateSupplyDTO): Promise<Supply> {
		...
		await db.runAsync(`UPDATE supplies SET ${fields.join(", ")} WHERE id = ?`, values);

		const updated = await SupplyService.getById(id);
	...
	static async delete(id: number): Promise<boolean> {
		const db = await getDatabase();
		const result = await db.runAsync("DELETE FROM supplies WHERE id = ?", [id]);
